...
*/
template <class T>
bool BTree<T>::BTree_insert(T value)
{
 ...
 */
template <class T>
void BTree<T>::SplitBlock(BTnode<T>* node_parent,int child_index,BTnode<T>* node_child)
{
...
*/
template <class T>
void BTree<T>::Notfull_insert(BTnode<T>* node,T value)
{
...
*/
template <class T>
bool BTree<T>::contain(T value)
{
    if (BTree_find(root,value) != nullptr)
        return true;
    return false;
}
...
*/
template <class T>
BTnode<T>* BTree<T>::BTree_find(BTnode<T>* node,T value)
{
    if (node == nullptr)//当前节点为空的时候
    {
        return nullptr;
    }
    else//当前节点不为空
...
*/
template <class T>
void BTree<T>::printpoint(BTnode<T>* node,int count)
{
    if (node != nullptr)//判断元素是否为空
    {
        int i,j;
        //每个节点从小到大打印
        for (i = 0;i < node->keyNum; i++)
...
*/
template <class T>
bool BTree<T>::BTree_delete(T value)
{
    if (!contain(value))
    {
        return false;
...
*/
template <class T>
void BTree<T>::MergeBlock(BTnode<T>* node_parent,int child_index)
{
...
*/
template <class T>
T BTree<T>::getprev(BTnode<T>* node)
{
...
*/
template <class T>
T BTree<T>::getnext(BTnode<T>* node)
{
    //在比节点位置大的节点中找一个最小的
    while (!node->isleaf)
    {
...
*/
template <class T>
void BTree<T>::BTree_deletebalance(BTnode<T>* node,T value)
{
    int i;
